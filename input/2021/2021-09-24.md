...
このコードはランタイムが Unix をターゲットにしていない場合には `Open(string path)` を使用できるが、もし使用者はランタイム固有の処理として書かなかった場合 `Open(string path)` を呼び出すコードを書くことになり、Unix 環境下ではメソッドが存在しないエラーとなる。このようなケースは Unix 環境下において `Open(string path)` を呼び出した場合に例外を投げるような形となるほうが望ましく、そういったケースを検出できる。
